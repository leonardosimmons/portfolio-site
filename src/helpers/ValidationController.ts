...
interface ValidationControls {
  ...
  contactMeForm: (fn: string, ln: string, sub: string, email: string, msg: string) => boolean;
};
...
class ValidationController implements ValidationControls
{
  ...
  // Controllers  
  public contactMeForm(fn: string, ln: string, sub: string, email: string, msg: string): boolean {
    let fail: string = '';
    fail = fail.concat(this.firstName(fn));
    fail = fail.concat(this.lastName(ln));
    fail = fail.concat(this.content(sub, 1, 'subject'));
    fail = fail.concat(this.email(email));
    fail = fail.concat(this.content(msg, 1, 'message'));
